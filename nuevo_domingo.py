...
main_dialog = uic.loadUiType("probando.ui")[0]
...
class LoadingApp(QtGui.QMainWindow, probando.Ui_MainWindow):
    pedido = pedidos()
    pedido.clear()
    ...
    def __init__(self):
        ...
        if atras: self.acomodar_pantalla()
        ...
        self.actionAbrir_Forumulario.triggered.connect(self.abrir_formulario)
        ...
        self.TE_Efectivo.textChanged.connect(self.previa_impresion)
        ...
        self.combo_box_forma_pago.addItem("Tarjeta")
        self.combo_box_forma_pago.addItem("Efectivo")
        self.combo_box_forma_pago.addItem("Transferencia")
        self.combo_box_forma_pago.addItem("Cortesia")
        self.combo_box_forma_pago.currentIndexChanged.connect(self.previa_impresion)

        self.combo_box_promo.addItem("3x2_sushi")
        self.combo_box_promo.addItem("3x2_bowls")
        self.combo_box_promo.currentIndexChanged.connect(self.previa_impresion)

        self.CB_Descuento.stateChanged.connect(self.previa_impresion)
        self.CB_Promo.stateChanged.connect(self.previa_impresion)

        for i in range(0,100,5):
            self.combo_box_descuento.addItem(str(i))

        self.combo_box_descuento.currentIndexChanged.connect(self.previa_impresion)
    ...
    def acomodar_pantalla(self):


        self.TE_Nombre.resize(120,30)
        self.TE_Telefono.resize(120,30)
        self.TE_Direccion.resize(120,50)
        self.TE_Uber.resize(120,30)
        self.TE_Comentario.resize(120,50)

        self.combo_box_entrega.resize(150,30)
        self.combo_box_forma_pago.resize(150,30)

        self.frame_cliente.setGeometry(10,50,200,450)


    def abrir_formulario(self):

        os.system("sudo python caja.py")
    ...
    def limpiar_orden(self):
        ...
        self.combo_box_entrega.setCurrentIndex(0)
        self.combo_box_forma_pago.setCurrentIndex(0)
        self.combo_box_descuento.setCurrentIndex(0)
 ...
 #       if self.checkBox_promo_lanz.checkState() or self.checkBox_promo_3x2.checkState():
        ...
        self.pedido.fecha_pedido = datetime.datetime.now()
        fecha = self.pedido.fecha_pedido.date()
        renglon += '{:02}'.format(fecha.day) + '-' + '{:02}'.format(fecha.month) + '-' + str(fecha.year)
        self.listWidget_ticket.addItem(renglon)

        if self.combo_box_forma_pago.currentText() == "Efectivo":
            self.lb_forma_de_pago.setText("Efectivo")
            self.pedido.forma_de_pago = "Efectivo"

        if self.combo_box_forma_pago.currentText() == "Tarjeta":
            self.lb_forma_de_pago.setText("Tarjeta")
            self.pedido.forma_de_pago = "Tarjeta"

        if self.combo_box_forma_pago.currentText() == "Transferencia":
            self.lb_forma_de_pago.setText("Transferencia")
            self.pedido.forma_de_pago = "Transferencia"

        if self.combo_box_forma_pago.currentText() == "Cortesia":
            self.lb_forma_de_pago.setText("Cortesia")
        ...
        if not self.combo_box_entrega.currentText() == "Uber":
            ...
            self.lb_forma_de_pago.setText("")
            self.pedido.forma_de_pago = "Transferencia"


        con_cuanto = self.TE_Efectivo.toPlainText()
        if self.combo_box_forma_pago.currentText() == "Efectivo" and len(con_cuanto) > 0:
            self.listWidget_ticket.addItem('')

            renglon_con_cuanto = 'Cancela con : ' + con_cuanto
        ...
        if self.combo_box_entrega.currentText() == "Salon":
            self.lb_forma_de_entrega.setText("Salon")
            renglon = 'Salon'
            self.listWidget_ticket.addItem(renglon)

        if self.combo_box_entrega.currentText() == "Para llevar":
            self.lb_forma_de_entrega.setText("Para Llevar")
            renglon = 'Para llevar en empaque de material 100% vegetal - 100% compostable.'
            self.listWidget_ticket.addItem(renglon)

        if self.combo_box_entrega.currentText() == "Terraza":
            self.lb_forma_de_entrega.setText("Terrza")
            renglon = 'Entregar en la terraza'
            self.listWidget_ticket.addItem(renglon)

        if self.combo_box_entrega.currentText() == "Express":
            self.lb_forma_de_entrega.setText("Express")
            renglon = 'Direccion : ' + self.TE_Direccion.toPlainText()
            self.listWidget_ticket.addItem(renglon)

        if self.combo_box_entrega.currentText() == "Uber":
            self.lb_forma_de_entrega.setText("Uber")
            renglon = 'Codigo UberEATS : ' + self.TE_Uber.toPlainText()
            self.listWidget_ticket.addItem(renglon)
...
#parte pedido
        ...
        for i in range(0,len(items_en_el_pedido)):
            ...
            # print nombre_item
            # print lista_precios[nombre_item][0]
            # print lista_precios[nombre_item][1]
            # print lista_precios[nombre_item][2]
            # print lista_precios[nombre_item][3]
            # print lista_precios[nombre_item][4]

            if lista_precios[nombre_item][1] == "bowl":
                cantidad_bowls += 1

            if lista_precios[nombre_item][1] == "bebida":
                cantidad_bebidas += 1

            if lista_precios[nombre_item][1] == "postre":
                cantidad_postres += 1
            ...
            if self.combo_box_entrega.currentText() == "Uber":
                ...
                indice_precio = 3
            valor = "{0:,}".format(float(lista_precios[nombre_item][indice_precio]))
            ...
            suma_bowl = float(lista_precios[nombre_item][indice_precio])

            for x in range(0, len(self.pedido.producto_pedido[items_en_el_pedido[i]])):


                suma_bowl = suma_bowl + float(lista_precios[self.pedido.producto_pedido[items_en_el_pedido[i]][x]][indice_precio])
                ...
                valor_adc = "{0:,}".format(float(lista_precios[self.pedido.producto_pedido[items_en_el_pedido[i]][x]][indice_precio]))
...
##################################################################
        ...
        self.TE_Bowls.setText(str(cantidad_bowls))
        self.TE_Bebidas.setText(str(cantidad_bebidas))
        self.TE_Postres.setText(str(cantidad_postres))



##################################################################

        if self.CB_Promo.isChecked():

            if self.combo_box_promo.currentText() == "3x2_sushi":

                cantidad_de_bowls_sushi = 0
                for i in range(0,len(items_en_el_pedido)):
                    nombre_item = items_en_el_pedido[i][3:len(items_en_el_pedido[i])]
                    if lista_precios[nombre_item][1] == "bowl" and lista_precios[nombre_item][0] == "sushi":
                        cantidad_de_bowls_sushi +=1

                promo = cantidad_de_bowls_sushi / 3
                ...
                for p in range(0, promo):
                    ...
                    for i in range(0, len(items_en_el_pedido)):
                        ...
                        if lista_precios[nombre_item][1] == "bowl" and lista_precios[nombre_item][0] == "sushi":
                            precio = float(lista_precios[nombre_item][3])
                    ...
                    #print items_en_el_pedido
                    #print items_en_el_pedido[item]

                    lista_promo.append(item)
                    nombre_descuento = items_en_el_pedido[item][3:len(items_en_el_pedido[item])]
                    descuento += float(lista_precios[nombre_descuento][3])

            if self.combo_box_promo.currentText() == "3x2_bowls":
...
##################################################################
        ...
        if self.CB_Descuento.isChecked() :
            self.listWidget_ticket.addItem('')
            renglon_descuento = '                        DESCUENTO'
            renglon_descuento += ' (' + self.combo_box_descuento.currentText() + '%)'

            porcentage_descuento = float(self.combo_box_descuento.currentText())
            descuento = porcentage_descuento * subotal/100
        ...
        if self.CB_Promo.isChecked():
            ...
            if self.combo_box_promo.currentText() == "3x2_sushi":
                renglon_descuento = '             PROMO SUSHI BOWL 3x2'
                espacios = [' '] * (48 - len(renglon_descuento))
                self.listWidget_ticket.addItem(renglon_descuento + ''.join(espacios) )
                self.listWidget_ticket.addItem('')

            if self.combo_box_promo.currentText() == "3x2_bowls":
                renglon_descuento = '                   PROMO 3x2 BOWL'
                espacios = [' '] * (48 - len(renglon_descuento))
                self.listWidget_ticket.addItem(renglon_descuento + ''.join(espacios) )
                self.listWidget_ticket.addItem('')

            renglon_descuento = '                  DESCUENTO PROMO'
        ...
        if self.CB_Descuento.isChecked() or self.CB_Promo.isChecked():
    ...
    def salvar_pedido(self):
        ...
        if self.CB_Promo.isChecked():
            promo = True
            if self.combo_box_promo.currentText() == "3x2_sushi" or self.combo_box_promo.currentText() == "3x2_bowl":
                ...
                if self.combo_box_promo.currentText() == "3x2_bowl":
                    cantidad_de_bowls_sushi = 0
                    for i in range(0, len(items_en_el_pedido)):
                        nombre_item = items_en_el_pedido[i][3:len(items_en_el_pedido[i])]
                        if lista_precios[nombre_item][1] == "bowl":
                ...
                cantidad_promo = cantidad_de_bowls_sushi / 3

                item = 0
                for p in range(0, cantidad_promo):
                    valor_minimo = 0
                    for i in range(0, len(items_en_el_pedido)):
                        nombre_item = items_en_el_pedido[i][3:len(items_en_el_pedido[i])]
                        ...
                        if (self.combo_box_promo.currentText() == "3x2_sushi" and lista_precios[nombre_item][1] == "bowl" and lista_precios[nombre_item][0] == "sushi") or _\
                                    (self.combo_box_promo.currentText() == "3x2_bowl" and lista_precios[nombre_item][1] == "bowl" ):
        ...
        for i in range(0, len(items_en_el_pedido)):
            ...
            if self.CB_Descuento.isChecked():
                porcentage_descuento = float(self.combo_box_descuento.currentText() )
                descuento = porcentage_descuento * suma_bowl/100
                suma_bowl = suma_bowl - descuento

            if self.CB_Promo.isChecked() and i in lista_promo :
                suma_bowl = suma_bowl -  float(lista_precios[nombre_item][2])

            registro += str(suma_bowl) + ','

            tipo_promo = ""
            if self.CB_Promo.isChecked() and len(lista_promo)>0:
                tipo_promo = str(self.combo_box_promo.currentText())

            registro += "'" + str(tipo_promo) + "',"
